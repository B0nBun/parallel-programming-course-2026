...
| :---------------------------------------------------- | ----: | -----: | -----: | -----: | -----: |
| **Этап 0:** Однопоточный baseline                     | 98.78 |      — |      — |      — |      — |
| **Этап 1:** Пустой лок (без записи)                   | 24.00 |  15.80 |   7.81 |   4.19 |   3.83 |
| **Этап 1:** Единый общий лок                          | 23.94 |  14.76 |   6.24 |   4.27 |   2.14 |
| **Этап 2:** Шардированный лок (16 групп)              | 17.52 |   5.62 |   5.47 |   3.72 |   3.70 |
| **Этап 3:** Thread-Local состояние (локальные данные) | 91.52 | 189.71 | 365.33 | 540.41 | 722.78 |
| **Этап 4:** Двойная буферизация                       | 33.56 |  67.17 | 134.38 | 220.40 | 331.31 |
...
| :---                                                  | :---:                             | :---:               | :---:               | :---:                                       |
| **Этап 2:** Шардированный лок                         | 99.8%                             | 9982 раз            | 1 раз               | 0                                           |
...
| Вопрос                                                                                                                                                                                                                                  | Ваш ответ |
| :---                                                                                                                                                                                                                                    | :--- |
| **1. Пустой лок против полного:** Почему пустой лок тоже резко падает с ростом потоков? Что добавляет полная версия?                                                                                                                    | От увеличения количества потоков нет выигрыша, так как все вызовы record в итоге становятся "последовательными", соответственно при выполнении нет никакой параллельности. При этом процессору все еще нужно тратить время на переключение контекста/пробуждение потока/конкуренцию за мьютекс. Полная версия добавляет, арифметические операции и подгрузку общих данных (buckets, sum, count, ...) за которые потоки также конкурируют. |
| **2. Провал шардирования и атомиков:** Почему 16 шардов и атомарная сводка на этапе 2 не спасли скорость записи на законе Ципфа? Куда переехало узкое место?                                                                            | Потому что все еще есть большая конкуренция за один и тот же лок, так как распределение ципфа подразумевает большую концентрацию на нулевом значении. Конкуренция за общие данные с атомиками также все еще осталась, куда соответственно и переехало узкое место. |
| **3. Замедление на этапе 3:** Почему на этапе 3 рост скорости замедляется после 8 потоков? Как это связано с выводом `lscpu`?                                                                                                           | Не все ядра процессора одинаковые. Производительные P-core ядра используются для высоконагруженных задач, тогда как энергоэффективные E-core ядра менее производительные. После заполнения потоками 8 ядер планировщих начинает использовать для новых потоков более "медленные" E-core ядра. Конкретно на моей машине нет явной маркировки разделения, но среди ядер есть отличия которые могут влиять на эффективность: 8 ядер имеют `MAXMHZ=1700`, а другие 8 `MAXMHZ=1200`. |
| **4. Двойная буферизация против TLS:** Почему этап 4 медленнее этапа 3 (в C++ в 2,5–3,5 раза, в Java в 1,1–1,7 раза, а на 16 потоках Java почти догоняет этап 3), но масштабируется почти линейно? В чём разница инструкций при записи? | Double buffering добавляет операцию "синхронизации" активного буфера при каждом record с `seq_cst` атомическими операциями, что тратит определенное время, однако остальная часть record все также работает с локальным состоянием без конкуренции с остальными потоками, поэтому это решение хорошо масштабируется. |
| **5. Эксперимент с шагом 3:** Что сломалось в тесте на этапе 4, когда вы убрали повторную проверку `active == b`? Почему итоговый `count` расходится с числом вызовов?                                                                  | При отсутствии проверки читатель (snapshot) мог переключить active буффер во время работы record, из-за чего record писал в буффер из которого одновременно читал снапшот. К тому же эти данные могли "потеряться" при сборе глобальной сводки. То есть ошибки вызваны гонкой данных. |
