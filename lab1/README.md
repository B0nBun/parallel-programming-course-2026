...
- **Процессор (CPU):** *[вставьте модель, например: AMD Ryzen 7 7840HS / Intel Core i7-14700HX]*
- **Число логических процессоров (`nproc`):** *[например: 16 или 28]*
- **Язык и версия:** *[например: C++20 (GCC 13.2) / Java 21 (OpenJDK)]*
...
| **Этап 1:** Пустой лок (без записи) | | | | | |
| **Этап 1:** Единый общий лок | | | | | |
| **Этап 2:** Шардированный лок (16 групп) | | | | | |
| **Этап 3:** Thread-Local состояние (локальные данные) | | | | | |
| **Этап 4:** Двойная буферизация | | | | | |

*Приложите сюда ссылку или файл со сводным графиком всех реализаций (млн оп/сек от числа потоков):*  
![Сводный график](путь_к_картинке.png)
...
| :--- | :---: | :---: | :---: | :---: |
| **Этап 2:** Шардированный лок | % | раз | раз | |
| **Этап 3:** Thread-Local состояние (локальные данные) | % | раз | раз | |
| **Этап 4:** Двойная буферизация | % | раз | раз | |
| **Этап 4 (без шага 3):** без проверки `active` | % | раз | раз | |

#### 4. Короткие ответы на вопросы (1–2 предложения прямо в таблице)

| Вопрос | Ваш ответ |
| :--- | :--- |
| **1. Пустой лок против полного:** Почему пустой лок тоже резко падает с ростом потоков? Что добавляет полная версия? | |
| **2. Провал шардирования и атомиков:** Почему 16 шардов и атомарная сводка на этапе 2 не спасли скорость записи на законе Ципфа? Куда переехало узкое место? | |
| **3. Замедление на этапе 3:** Почему на этапе 3 рост скорости замедляется после 8 потоков? Как это связано с выводом `lscpu`? | |
